...
3.  The new algorithm produces a different result when outputting a clean dataset without duplicates or incomplete registries.

    The old script (uses package `move`) takes the raw data of Vulture "Cuba-7198" (that has 1.511.956 observations) and leaves it with 302.309 elements after cleaning it.

    The new script (uses package `move2`) outputs 302331 elements after the clean process is over, meaning that 21 observation don't get picked by this algorithm.
